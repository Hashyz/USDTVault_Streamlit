...
async function testBlockchainIntegration() {
  ...
  try {
    ...
    if (parseFloat(usdtFormatted) > 0 || parseFloat(bnbFormatted) > 0) {
      ...
      console.log(`   USDT Balance: $${parseFloat(usdtFormatted).toFixed(2)}`);
      console.log(`   BNB Balance: ${bnbFormatted} BNB ($${bnbValueUSD.toFixed(2)})`);
    } else {
      console.log('\n⚠️  The wallet appears to be empty.');
      console.log('   The user mentioned adding $10 for testing.');
      console.log('   Please verify:');
      console.log('   1. The correct wallet address is being used');
      console.log('   2. The funds were sent to BSC network (not another chain)');
      console.log('   3. The funds were sent as USDT BEP20 token');
    }
